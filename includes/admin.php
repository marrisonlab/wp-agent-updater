    ...
    public function render_backups_page() {
        ...
        if (isset($_POST['action']) && isset($_POST['backup_file'])) {
            ...
            if ($_POST['action'] === 'restore') {
                ...
                if (is_wp_error($res)) {
                    echo '<div class="notice notice-error is-dismissible"><p>Errore Ripristino: ' . $res->get_error_message() . '</p></div>';
                } else {
                    echo '<div class="notice notice-success is-dismissible"><p>Ripristino completato con successo!</p></div>';
                }
            } elseif ($_POST['action'] === 'delete') {
                if ($backups_instance->delete_backup($file)) {
                    echo '<div class="notice notice-success is-dismissible"><p>Backup eliminato.</p></div>';
                } else {
                    echo '<div class="notice notice-error is-dismissible"><p>Errore eliminazione backup.</p></div>';
                }
            }
        }
        ...
        <div class="wrap">
            ...
            <p>Qui puoi gestire i backup automatici creati prima degli aggiornamenti.</p>
            ...
            <table class="wp-list-table widefat fixed striped">
                ...
                <tbody>
                    <?php if (empty($backups)): ?>
                        <tr><td colspan="5">Nessun backup trovato.</td></tr>
                    <?php else: ?>
                        <?php foreach ($backups as $b): ?>
                            <tr>
                                ...
                                <td>
                                    <form method="post" style="display:inline;" onsubmit="return confirm('Sei sicuro di voler ripristinare questo backup? Sovrascriverà i file attuali.');">
                                        ...
                                        <button type="submit" class="button button-primary">Ripristina</button>
                                    </form>
                                    &nbsp;
                                    <form method="post" style="display:inline;" onsubmit="return confirm('Eliminare definitivamente questo backup?');">
                                        <?php wp_nonce_field('wp_agent_updater_backup_action'); ?>
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="backup_file" value="<?php echo esc_attr($b['filename']); ?>">
                                        <button type="submit" class="button button-link-delete" style="color: #a00;">Elimina</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    public function render_page() {
        ...
        <div class="wrap">
            ...
            <p class="wp-agent-updater-dashboard-intro">
                Gestisci lo stato del servizio, la connessione al Master e le azioni rapide di aggiornamento da un unico pannello chiaro e compatto.
            </p>
                ...
                <div class="wp-agent-updater-dashboard-grid">
                        ...
                        <div class="wp-agent-updater-card">
                            <div class="wp-agent-updater-card-header">
                                ...
                                <span id="service-status-text" class="wp-agent-updater-status-badge <?php echo get_option('wp_agent_updater_active') === 'yes' ? 'status-active' : 'status-inactive'; ?>">
                                    <?php echo get_option('wp_agent_updater_active') === 'yes' ? 'ATTIVO' : 'DISATTIVO'; ?>
                                </span>
                            </div>
                            <p class="wp-agent-updater-card-subtitle">
                                Attiva il WP Agent Updater per consentire al Master di monitorare il sito ed eseguire aggiornamenti centralizzati.
                            </p>
                                ...
                                <tr valign="top">
                                    ...
                                    <td>
                                        <div class="wp-agent-updater-inline-field">
                                            ...
                                            <span><?php echo get_option('wp_agent_updater_active') === 'yes' ? esc_html__('Servizio attivo', 'wp-agent-updater') : esc_html__('Servizio disattivo', 'wp-agent-updater'); ?></span>
                                            <span id="save-spinner" class="spinner" style="float:none;"></span>
                                        </div>
                                        <p class="description">Le modifiche vengono salvate automaticamente tramite AJAX, senza ricaricare la pagina.</p>
                                    </td>
                                </tr>
                            </table>
                        </div>
                        ...
                        <div class="wp-agent-updater-card">
                            <div class="wp-agent-updater-card-header">
                                <div class="wp-agent-updater-card-title">
                                    ...
                                    <span>Configurazione Master</span>
                                </div>
                            </div>
                            <p class="wp-agent-updater-card-subtitle">
                                Imposta l'URL del sito Master che controllerà questo agente. L'URL viene memorizzato in modo sicuro.
                            </p>
                                ...
                                <table class="form-table">
                                    <tr valign="top">
                                        ...
                                        <td>
                                            ...
                                            <p class="description">Inserisci l'URL completo del sito Master (consigliato HTTPS).</p>
                                            <?php if ($master_url): ?>
                                                <p class="description" style="color: #46b450; margin-top: 5px;">
                                                    <span class="dashicons dashicons-yes"></span> URL configurato correttamente.
                                                </p>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                </table>
                                <?php submit_button('Salva Configurazione URL'); ?>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="wp-agent-updater-dashboard-side">
                        <div class="wp-agent-updater-card">
                            ...
                            <p class="wp-agent-updater-card-subtitle">
                                Esegui una sincronizzazione immediata e verifica rapidamente la versione installata del plugin.
                            </p>

                            <form method="post" style="margin-bottom: 16px;">
                                <input type="hidden" name="force_sync" value="1">
                                <?php submit_button('Forza Sincronizzazione', 'secondary'); ?>
                            </form>

                            <hr style="margin: 16px 0; border: 0; border-top: 1px solid #eee;">

                            <h3 style="margin-top: 0;">Aggiornamenti Plugin</h3>
                            <p>Versione installata: <strong><?php echo get_plugin_data(WP_AGENT_UPDATER_PATH . 'wp-agent-updater.php')['Version']; ?></strong></p>
                            ...
                            if (response.success) {
                                if ($statusText) {
                                    if (isChecked) {
                                        ...
                                        $statusText.text('DISATTIVO').removeClass('status-active').addClass('status-inactive');
                                    }
                                }
                            } else {
